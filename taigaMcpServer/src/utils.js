...
export function formatUserStoryList(userStories) {
  return userStories.map(us =>
    `- #${us.ref} [id:${us.id}]: ${us.subject}
  Status: ${getSafeValue(us.status_extra_info?.name)}
  Assigned: ${getSafeValue(us.assigned_to_extra_info?.full_name, STATUS_LABELS.UNASSIGNED)}
  Sprint: ${getSafeValue(us.milestone_extra_info?.name, STATUS_LABELS.NO_SPRINT)}`
  ).join('\n\n');
}
