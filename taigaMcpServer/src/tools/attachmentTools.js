...
const ITEM_ENDPOINTS = {
  ...
  task: API_ENDPOINTS.TASKS || '/tasks',
};

async function embedImageInDescription(itemType, itemId, attachmentId, attachmentUrl, attachmentName, position = 'append') {
  const client = await createAuthenticatedClient();
  const endpoint = `${ITEM_ENDPOINTS[itemType]}/${itemId}`;
  const { data: item } = await client.get(endpoint);
  const refreshedUrl = attachmentUrl.includes('#_taiga-refresh')
    ? attachmentUrl
    : `${attachmentUrl}${attachmentUrl.includes('?') ? '' : '?'}#_taiga-refresh=userstory:${attachmentId}`;
  const imageMd = `![${attachmentName}](${refreshedUrl})`;
  const currentDesc = item.description || '';
  const newDesc = position === 'prepend'
    ? `${imageMd}\n\n${currentDesc}`.trim()
    : `${currentDesc}\n\n${imageMd}`.trim();
  await client.patch(endpoint, { description: newDesc, version: item.version });
}
...
export const uploadAttachmentTool = {
  ...
  schema: {
    ...
    embedInDescription: z.boolean().optional().describe('If true, also embed the uploaded image inline in the item description as markdown'),
    embedPosition: z.enum(['append', 'prepend']).optional().describe('Where to insert the embedded image (default: append)')
  },

  handler: async ({ itemType, itemId, projectIdentifier, fileData, fileName, mimeType, filePath, description, embedInDescription, embedPosition }) => {
    try {
      ...
      let embedNote = '';
      if (embedInDescription) {
        try {
          await embedImageInDescription(itemType, actualItemId, uploadResult.id, uploadResult.url, uploadResult.name, embedPosition || 'append');
          embedNote = `\n- Embedded in description: ✅ (${embedPosition || 'append'})`;
        } catch (e) {
          embedNote = `\n- Embedded in description: ❌ (${e.message})`;
        }
      }
