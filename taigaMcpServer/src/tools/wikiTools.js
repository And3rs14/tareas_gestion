...
async function createWikiLink(projectId, title, href) {
  const client = await createAuthenticatedClient();
  await client.post('/wiki-links', { project: projectId, title, href });
}
...
export const createWikiPageTool = {
  ...
  schema: {
    ...
    sidebarTitle: z.string().optional().describe('If provided, also add this page to the wiki sidebar with this title'),
  },

  handler: async ({ project, slug, content, watchers, sidebarTitle }) => {
    try {
      ...
      const wikiData = { project: projectId, slug, content, watchers: watchers || [] };
      const result = await taigaService.createWikiPage(wikiData);

      let sidebarNote = '';
      if (sidebarTitle) {
        try {
          await createWikiLink(projectId, sidebarTitle, result.slug);
          sidebarNote = `\n- Sidebar link: ✅ "${sidebarTitle}"`;
        } catch (e) {
          sidebarNote = `\n- Sidebar link: ❌ ${e.message}`;
        }
      }
      ...
      return createSuccessResponse(
        ...
        `- Watchers: ${result.watchers?.length || 0}` +
        sidebarNote
      );
    } catch (error) {
...
export const listWikiPagesTool = {
  ...
  handler: async ({ project }) => {
    try {
      ...
      return createSuccessResponse(
        `📖 **${projectId} - Wiki頁面列表** (${wikiPages.length}個)\n\n` +
        wikiList
      );
    } catch (error) {
  ...
  handler: async ({ project, identifier }) => {
    try {
      ...
      if (typeof identifier === 'number') {
        ...
        wikiPage = await taigaService.getWikiPageBySlug(identifier, projectId);
      }
