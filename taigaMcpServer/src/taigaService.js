...
export class TaigaService {
   ...
   */
  async getUserStory(userStoryId, projectId = null) {
    try {
      const client = await createAuthenticatedClient();
      if (projectId) {
   ...
   */
  async moveUserStoryInKanban(storyId, position) {
    const client = await createAuthenticatedClient();
    const story = await this.getUserStory(storyId);
    const allStories = await this.listUserStories(story.project);
    const sameStatus = allStories
      .filter(s => s.status === story.status)
      .sort((a, b) => a.kanban_order - b.kanban_order);

    const otherIds = sameStatus.filter(s => s.id !== story.id).map(s => s.id);
    let orderedIds;
    if (position === 'top') {
      orderedIds = [story.id, ...otherIds];
    } else if (position === 'bottom') {
      orderedIds = [...otherIds, story.id];
    } else {
      // numeric index (0-based)
      const idx = Math.min(parseInt(position), sameStatus.length - 1);
      otherIds.splice(idx, 0, story.id);
      orderedIds = otherIds;
    }

    const response = await client.post(`${API_ENDPOINTS.USER_STORIES}/bulk_update_kanban_order`, {
      project_id: story.project,
      status_id: story.status,
      bulk_userstories: orderedIds
    });
    return { ref: story.ref, subject: story.subject, status_extra_info: story.status_extra_info, position: orderedIds.indexOf(story.id) + 1 };
  }
